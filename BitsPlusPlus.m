...
% value, whereas 'data' must be a 248 element row vector of bytes. 
%
...
% 04.04.2014 Added goggle and DAC control for Bits# with DIOCommand. (JT)
...
if strcmpi(cmd, 'DIOCommand')
    ...
    % optional shutter goggle control for Bits# devices
    if nargin >= 9,
        goggle = varargin{6};
        if size(goggle,1)~=1 || size(goggle,2)~=2,
            error('Goggle input must be a 1 by 2 vector');
        end
        if goggle(1)~=1 && goggle(1)~=0 || goggle(2)~=1 && goggle(2)~=0,
            error('Goggle inputs must be zeros or ones');
        end
    else
        goggle = [0 0];
    end
    
    % optional DAC control for Bits# devices
    if nargin >= 10,
        DAC = varargin{7};
        if size(DAC,1)~=1 || size(DAC,2)~=2,
            error('DAC input must be a 1 by 2 vector');
        end
        if DAC(1)~=1 && DAC(1)~=0 || DAC(2)~=1 && DAC(2)~=0,
            error('DAC inputs must be betwenn -5 and 5 (Volts)');
        end
    else
        DAC = [0 0];
    end
    ...
    for i=1:size(mask, 1)
        ...
        tlockdata = BitsPlusDIO2Matrix(mask(i), data(i,:), command(i),goggle(i,1:2),DAC(i,1:2));

        % Convert from Matlab matrix to OpenGL pixel format:
        if ~isempty(goggle) && ~isempty(DAC),
            encodedDIOdata = uint8(zeros(3, 514));
        else
